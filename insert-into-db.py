...
worksheet = workbook["Sheet1"]  #change sheet name
...
for row in worksheet.iter_rows(min_row=2, values_only=True):
    ...
    try:
        ...
        billing_date = convert_date(values[7])
        created_date = convert_date(values[10])

        query = """
            INSERT INTO billing_account (
                service_provider, account_name, account_number, 
                category, paybill_number, ussd_number, 
                frequency, billing_date, bill_amount, 
                account_owner, created_date, invoice_number, 
                status, bank_account
        ...
        cur.execute(query, (
            ...
            values[6],  # frequency
            billing_date,
            values[8],  # bill_amount
            values[9],  # account_owner
            created_date,
            values[11],  # invoice_number
            values[12],  # status
            values[13]  # bank_account
        ))
    except Exception as e:
